 ...
 * @file Movie.h
 * @author Luc Debaupte
 * @brief Header files for Movie, and its subclasses (including Comedy, Drama, Classic)
 * @date 2022-11-13
 *
 */
#pragma
...
{
public:
    void createMovie(int stock, std::string director, std::string title, int yearRelease)
    {
        setStock(stock);
...
{
public:
    void createMovie(int stock, std::string director, std::string title, std::string majorActor, std::string releaseDate)
    {
        setStock(stock);
        setDirector(director);
        setTitle(title);
